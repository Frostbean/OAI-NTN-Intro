...
|-- | -- | --|
|$T_{TA}$ | Timing Advance | 提前發送量。UE 必須比預期時間更早發送訊號，以補償長距離傳輸延遲。|
|$N_{TA}$ | Timing Advance Offset | OAI 中用於調整上行鏈路定時的偏移值，NTN 的數值遠大於地面網路。|
|$K_{offset}$ | Scheduling Offset | 調度偏移。考慮到衛星往返延遲（RTT），gNB 指令發出後，UE 需要額外的等待時間才能執行。|
|Common TA | Common Timing Advance | 衛星覆蓋範圍內所有 UE 共有的延遲部分（通常指衛星到網關的距離）。|
|UE-specific TA | UE-specific Timing Advance | 個別 UE 因位置不同而產生的額外延遲補償。|
...
OAI 對於 NTN 標準的支援始於歐洲太空總署 ESA 支持的兩個計畫：5G-GOA 及 5G-LEO。

|計畫|5G-GOA|5G-LEO|
|-|-|-|
|目標|根據 3GPP Rel-17，於 OAI 上實作 NTN 所需的修改，完成 GEO 衛星的對接測試|繼續在 OAI 程式碼上延伸至支援 Transparent Payload with LEO Satellite|
...
|成果|開發並發布於 OAI [goa-5g-ntn](https://gitlab.eurecom.fr/oai/openairinterface5g/-/commits/goa-5g-ntn) 分支，後續整合進 develop 分支|開發並發布於 develop 分支|

Release 17 在 2022 年 4 月凍結。

5G-GOA 計畫時間為 2021 年 2 月至 2024 年 4 月，

5G-GOA 根據 3GPP 的討論及結果，實作 PHY 及更上層的技術，包括同步、timers 及 random access procedure 等，主要支援 GEO 衛星的 NTN 通訊。

5G-GOA 基於 OAI 的開源程式碼，開發於與 OAI develop 分支獨立的 [goa-5g-ntn](https://gitlab.eurecom.fr/oai/openairinterface5g/-/commits/goa-5g-ntn)，並於計畫之後將程式碼透過 merge request 整合進 develop 分支。
...
https://gitlab.eurecom.fr/oai/openairinterface5g/-/merge_requests/2712
https://gitlab.eurecom.fr/oai/openairinterface5g/-/merge_requests/2893
...
參考 [How to run a NTN configuration](https://gitlab.eurecom.fr/oai/openairinterface5g/-/blob/develop/doc/RUNMODEM.md#how-to-run-a-ntn-configuration) 可以找到 GEO 及 LEO 衛星模擬的設定。
